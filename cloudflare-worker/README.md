...
Heb je al eerder zo'n token aangemaakt (bijvoorbeeld voor een eerdere
poging)? Dan kun je die gewoon hergebruiken in stap 3 — hoeft niet
opnieuw.

1. Ga naar GitHub → **Settings** (je account) → **Developer settings** → **Personal access tokens** → **Fine-grained tokens** → **Generate new token**. (Directe link: `https://github.com/settings/tokens?type=beta`.)
